      ...
      <Link to={`/subjects/${subject.id}`} className="group flex h-full flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-gray-800 dark:bg-gray-900 dark:hover:shadow-gray-900/60" aria-label={`${subject.title} — ${progressValue}% complete`}>
        ...
        <div className="flex flex-1 flex-col p-5 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            <span className="flex items-center gap-1"><span className="font-medium text-gray-700 dark:text-gray-300">{chapterCount}</span> {chapterCount === 1 ? 'chapter' : 'chapters'}</span>
          ...
          {totalCount > 0 && (
            <div className="space-y-1">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 tabular-nums">{completedCount} / {totalCount} sections complete</p>
            </div>
          )}
        </div>
      </Link>
